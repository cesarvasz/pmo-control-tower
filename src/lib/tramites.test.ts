...
describe("filtrarExpedientes", () => {
  const exps = construirExpedientes([
    fila({ c807_file: "F1", Creado: "2026-01-05T08:00:00", Mesa: "Mesa 1", Usuario: "ANA", Analista: "BETO", Cliente: "C1", Proceso: "Aduana", Docalpha: "Ducafast", Impexp: "3" }),
    fila({ c807_file: "F2", Creado: "2026-02-05T08:00:00", Mesa: "Mesa 2", Usuario: "CARLA", Analista: "DIEGO", Cliente: "C2", Proceso: "Importación", Docalpha: "No Ducafast", Impexp: "2" }),
  ]);
  ...
  it("filtra por mes, mesa, cliente, proceso y Ducafast", () => {
    ...
    // Impexp: la hoja guarda el código (2/3) y se filtra por la etiqueta traducida.
    expect(filtrarExpedientes(exps, f({ impexp: ["Exportación"] })).map((e) => e.file)).toEqual(["F2"]);
    expect(filtrarExpedientes(exps, f({ impexp: ["Importación"] })).map((e) => e.file)).toEqual(["F1"]);
  });
