...
export default function LineaProduccionIAPage() {
  ...
  useEffect(() => {
    ...
    fetch("/valor/linea-produccion-ia.html")
      .then((r) => r.text())
      .then((html) => {
        if (!mounted || !rootRef.current) return;

        // Scope the two problematic global selectors; all class/id selectors
        // are page-specific and don't conflict with the app.
        const cssText = [...html.matchAll(/<style[^>]*>([\s\S]*?)<\/style>/g)]
          .map((m) => m[1])
          .join("\n")
          .replace(/^(\s*)\*(\s*\{)/gm, "$1.lpia-root *$2")
          .replace(/^(\s*)body(\s*\{)/gm, "$1.lpia-root$2")
          .replace(/body\[data-theme/g, ".lpia-root[data-theme");
  ...
  return (
    <div
      ref={rootRef}
      className="lpia-root rounded-xl border"
