/**
 * ROI Digitalización OCR — PMO Control Tower
 * ---------------------------------------------------------------------------
 * Expone en SOLO LECTURA (doGet) la pestaña "009" de un archivo de Google Sheets
 * DISTINTO al de 003 y al de Clonación, para que la app la muestre en la pestaña
 * "Digitalización OCR" de la página ROI. Los encabezados se leen de la fila 1,
 * así que las columnas nuevas fluyen solas sin tocar este archivo.
 *
 * Fuente INDEPENDIENTE de 003 (roi-log.gs) y de Clonación (roi-clonacion.gs):
 * su propio archivo de origen, su propio doGet, su propia variable de entorno
 * (ROI_OCR_WEBAPP_URL), su propia Script Property (OCR_SHEET_ID) y su propio
 * caché en Drive. No comparte código con los otros — así un cambio en un script
 * no puede romper otro.
 *
 * Requiere Script Properties: OCR_SHEET_ID (el ID del archivo de Google Sheets
 * de la digitalización OCR — de su URL: .../spreadsheets/d/<ESTE_ID>/edit. NO es
 * el mismo archivo que 003 ni que Clonación).
 * Proyecto → Configuración (⚙️) → Propiedades del script.
 *
 * Todas las reglas de negocio (ventana hábil L–V 08:00–18:00, cálculo de T1 y
 * T2, estadística) viven en la app, en src/lib/ocrHabil.ts +
 * src/lib/digitalizacion.ts — este script SOLO codifica para transporte, igual
 * que roi-log.gs y roi-clonacion.gs. No agrupa, no deduplica, no calcula nada:
 * una fila de la pestaña es un expediente, y así llega a la app.
 * ---------------------------------------------------------------------------
 */

var SHEET_TAB = "009"; // nombre de la pestaña a leer — actualizar aquí si cambia.

var PROP_CACHE_ID = "ROI_OCR_CACHE_FILE_ID"; // lo escribe el script, no lo pongas a mano
var CACHE_NOMBRE = "roi-ocr-cache.json";
...
function regenerarCache() {
  ...
  if (hoja.aviso && !hoja.filas.length && /OCR_SHEET_ID|no existe la pestaña/.test(hoja.aviso)) {
    throw new Error("ROI OCR: no se regeneró el caché (se conserva el anterior) — " + hoja.aviso);
  }
...
var COLS_TEXTO = ["c807_file", "Cliente", "Embarque"];
/** Columnas de fecha → van como segundos desde EPOCA. */
var COLS_FECHA = ["Creacion", "Digit_docs", "Digit_carta_licencia"];
 ...
 * cual (aquí solo `id`, numérico), luego un índice al diccionario por cada
 * columna de texto repetido, luego los segundos de cada fecha (`Creacion`,
 * `Digit_docs`, `Digit_carta_licencia`) — null si el hito no ocurrió.
 *
 * Una fila = un expediente. `c807_file` es la llave de negocio y no se repite;
 * el script no agrupa ni deduplica — eso es del dominio de la app.
 */
function leerHoja() {
  ...
  var id = PropertiesService.getScriptProperties().getProperty("OCR_SHEET_ID");
  if (!id) return vacio("falta la Script Property OCR_SHEET_ID (ID del archivo de la digitalización OCR, distinto al de 003)");
 ...
 * Responde "¿por qué la app no muestra datos?": si el OCR_SHEET_ID está puesto,
...
function diagnosticarHoja() {
  ...
  Logger.log("OCR_SHEET_ID          : " + (id || "✗ NO CONFIGURADO"));
